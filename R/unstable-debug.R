...
# calculate rho
# v <- 1 + 2 * (1 - RHO) * 1 -- this is what it was before
v <- 1 + (1 + (RHO)**2 - 2 * (RHO)) + 2*RHO - 1 # this one kinda works
# v <- 1 + (1 - RHO)**2 -- this is just an alternative to above
# this one is with the new derivation, can't be right
v <- 1/RHO**2 + (1/RHO**2 - 2 * 1/RHO + 1)
# but RHO**2 + (1/RHO**2 - 2 * 1/RHO + 1) = 1.25
...
# TRYING THIS WITH A NEW VARIANCE FOR F1

v1 <- RHO**2
f1 <- dnorm(grid_x, sd=sqrt(v1))
u1.int <- uniroot(function(u) sum(f1[abs(grid_x) >= u]*delta) - as[1],
                  c(0, 100))$root
u1.int
f1.cont <- f1
